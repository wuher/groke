...
(function () {
     ...
     if (typeof require === 'undefined') {
         require = function (module) {
             var i, mod, modulepath = module.split("/");
             mod = groke.modules[modulepath[0]];
             for (i = 1; i < modulepath.length; i += 1) {
                 mod = mod[modulepath[i]];
             }
             return mod;
         };
     }
...
groke.handleResponse = function (response) {
    ...
    case 'function':
        return function () {
            ...
            return groke.ajax(groke.uriPrefixes.functions + response.resid,
    ...
    case 'object':
        ...
        for (i = 0; i < response.data[1].length; i += 1) {
            ...
            ret[funcname] = function (funcname) {
                var callback, args = Array.prototype.slice.call(arguments, 1);
                if (typeof args[args.length-1] === 'function') {
                    callback = args.pop();
                }
                return groke.ajax(groke.uriPrefixes.objects +
    ...
    if (response.exc_info) {
        throw new Error("Error on server: " + response.exc_info);
    } else {
